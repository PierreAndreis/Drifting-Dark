...
const calculateTier = heroes => {
  let winRateAvg =
    heroes.reduce((prev, next) => prev + next.winRate, 0) / heroes.length;
  let banRateAvg =
    heroes.reduce((prev, next) => prev + next.banRate, 0) / heroes.length;
  let pickRateAvg =
    heroes.reduce((prev, next) => prev + next.pickRate, 0) / heroes.length;
  ...
  return heroes.map(h => {
    ...
    if (
      (relative.winRate > 1 &&
        (relative.banRate >= 3 || relative.pickRate >= 1.25)) ||
    ...
    // then it is bad
    if (
      (relative.winRate <= 1 && relative.banRate <= 0.5) ||
      (relative.winRate <= 0.5 &&
        (relative.banRate > 0.1 || relative.pickRate > 0.1))
    ) {
      tier = 0;
    }

    // now detect op and average bad
    if (tier === 3 && (relative.winRate >= 1.08 || relative.banRate >= 4))
      tier = 4; // OP!
    ...
    if (
      tier === 0 &&
      (relative.winRate >= 0.9 && relative.pickRate >= 0.4)
    )
      tier = 1; // not so bad...
...
class VGHeroes extends BaseCouchbase {
  async heroListStats(region) {
    ...
    ["winRate", "banRate", "pickRate"].forEach(property => {
      ...
      heroes = heroes.map((h, index) => ({
        ...h,
        rank: { ...h.rank, [property]: index + 1 }
      }));
    });
    ...
    CacheService.hashSet(
      `Heroes:History:${PATCH}:${region || "all"}`,
      today,
      { date: new Date().toString(), patchVersion: PATCH, heroes: heroes },
      true
    );
  ...
  async getHistoricalByDate(heroName, region, amount) {
    ...
    while (amountAlready < amount) {
      let dateToFind = new Date(
        new Date().setDate(today.getDate() - amountAlready)
      );
      ...
      if (start < dateToFind && end >= dateToFind) {
        dates[patch.version] = merge(dates[patch.version], [
          dateToFind.toLocaleDateString()
        ]);
        amountAlready++;
      } else currentPatch++;
    }
        ...
        CacheService.hashGet(
          ...
          return r.map((result, i) => {
            ...
            if (heroes && heroName !== "list") {
              heroes =
                result &&
                result.heroes.find(hero => hero.name === heroName);
            }
  ...
  async getHistoricalByPatch(heroName, region, amount) {
    ...
    let patchesToSearch = sortBy(Patches, false, "startAt").slice(
      0,
      amount
    );
    ...
    let heroes = patchesToSearch.map(async update => {
      ...
      let patchRes = await CacheService.hashGet(
        `Heroes:History:${update.version}:${region || "all"}`,
  ...
  getHeroStats(heroName, region) {
    const get = async () => {
      ...
      let merged = list.reduce((p, now) => merge(p, now.heroes), {
        totalGames
      });
      ...
      if (transformed) {
        ...
        transformed.stats.forEach(stat => {
          let leaderboard = new LeaderboardService(
            `Heroes:${PATCH}:${stat.name}`,
            region
          );
          promisesLeaderboard.push(
            leaderboard.updateAndGet(heroName, stat.stats)
          );
    ...
    return CacheService.preferCache(key, get, {
      expireSeconds:
        Config.CACHE.REDIS_HEROES_STATS_INDIVIDUAL_CACHE_EXPIRE
    });
  }

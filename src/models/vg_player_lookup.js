...
class VGPlayerLookup {
  ...
  async findPlayerAPI({ playerName, playerId }) {
    ...
    Config.VAINGLORY.REGIONS.forEach(r =>
      regionsCalls.push(
        VaingloryService.getPlayer({ playerId, playerName, region: r })
      )
    );
  ...
  async getByName(playerName, region) {
    try {
      ...
      const get = async () => {
        ...
        if (DEBUG)
          logger.debug(
            `[LOOKUP] FOUND ${playerName} AT ${res.raw.attributes.shardId}`
          );
  ...
  async getById(playerId, region) {
    try {
      ...
      const get = async () => {
        ...
        if (DEBUG)
          logger.debug(
            `[LOOKUP] FOUND ${playerId} AT ${res.raw.attributes.shardId}`
          );

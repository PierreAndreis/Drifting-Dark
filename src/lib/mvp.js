...
export default (players, rosters, gameMode) => {
  ...
  players.forEach(p => {
    ...
    kdaRelative.push((p.kills + p.assists) / (Math.pow(p.deaths, 0.75) || 1));
  ...
  let kdaRelativeAvg = kdaRelative.reduce((v, n) => v + n, 0) / (kdaRelative.length || 1);
  
  // lodash.forEach(kdaRelative, (kdaSide, index) => {
  //   kdaRelativeAvg[index] = kdaRelative[index].reduce((v, n) => v + n, 0) / (kdaRelative[index].length || 1);
  // });
